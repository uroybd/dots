...
def zellij-smart-rename [] {
    ...
    let active_tab_info = (
        try {
            zellij action current-tab-info --json e> /dev/null | from json
        } catch { {} }
    )
    ...
    let is_git = (
        try {
            git rev-parse --is-inside-work-tree e> /dev/null | str trim
        } catch { "false" }
    ) == "true"
    let is_automated = (
...
source $"($nu.default-config-dir)/fnm.nu"
source $"($nu.default-config-dir)/atuin.nu"
source $"($nu.default-config-dir)/zoxide.nu"
source $"($nu.default-config-dir)/carapace.nu"
source $"($nu.default-config-dir)/jira.nu"
source $"($nu.default-config-dir)/gh.nu"
source $"($nu.default-config-dir)/misc.nu"
source $"($nu.default-config-dir)/commitart.nu"
source $"($nu.default-config-dir)/todotxt.nu"
