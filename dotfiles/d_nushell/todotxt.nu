...
def todotxt-render-task [task: record] {
    ...
    if $task.priority != null {
        $color = (
            $todotxt_priority_colors
            | get ($task.priority | str uppercase)
            | default "wb"
        )
    }
    ...
    $title = ($title | str replace --all --regex '(@\S+)' $"(ansi lmb)$1(ansi $color)")
    $title = ($title | str replace --all --regex '(\+\S+)' $"(ansi lcb)$1(ansi $color)")
    if $task.rec != null {
        $title = $"($title) (ansi gb)(ansi reset)"
    }
    if $task.due != null {
