...
# 8) and --block the string drawn for each cell (default "██"). Since one ANSI
# color usually matches the terminal background, --bg names that color index and
# --bg-with the color drawn in its place (default: 0 -> 8). --offset left-pads
# every row with that many blank cells.
def "commitart" [
    --cols: int = 8
    --block: string = "██"
    --bg: int = 0
    ...
    let pad = (
        0..<($offset * ($block | str length --grapheme-clusters))
        | each { " " }
        | str join
    )
    ...
    for c in $chars {
        ...
        $out = $out + (ansi -e $"38;5;($v)m") + $block + (ansi reset)
        $i = $i + 1
        if ($i mod $cols) == 0 {
            $out = $out + "\n"
        }
...
def "commitart repo" [
    ...
    if (git rev-parse --is-inside-work-tree e> /dev/null | str trim) != "true" {
